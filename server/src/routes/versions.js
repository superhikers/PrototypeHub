...
router.get('/projects/:pid/versions', (req, res, next) => {
  try {
    ...
    const versions = db.prepare(
      'SELECT v.*, (SELECT COUNT(*) FROM annotations WHERE version_id = v.id) AS annotation_count FROM prototype_versions v WHERE v.project_id = ? ORDER BY v.version_number DESC'
    ).all(req.params.pid);
...
router.post('/projects/:pid/versions', upload.single('file'), (req, res, next) => {
  try {
    ...
    const maxVer = db.prepare('SELECT MAX(version_number) AS max_ver FROM prototype_versions WHERE project_id = ?').get(req.params.pid);
